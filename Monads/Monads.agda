...
return-law1 : ∀ {ℓ} (M :{#} Premonad ℓ) → IsMonad M → {X Y :{#} Set ℓ} →  {x : X} → {k : X → type M Y}
                                                  → bind M (return M x) k ≡ k x
return-law1 M Mmon = ¶fst(unmonad Mmon)

return-law2 : ∀ {ℓ} (M :{#} Premonad ℓ) → IsMonad M → {X :{#} Set ℓ} → {fx : type M X} → bind M fx (return M) ≡ fx
return-law2 M Mmon = ¶fst(¶snd(unmonad Mmon))

assoc-law : ∀ {ℓ} (M :{#} Premonad ℓ) → IsMonad M → {X Y Z :{#} Set ℓ} {fx : type M X} {k : X → type M Y} {q : Y → type M Z}
                    → bind M (bind M fx k) q ≡ bind M fx (λ x → bind M (k x) q)
assoc-law M Mmon = ¶fst(¶snd(¶snd(unmonad Mmon)))
